...
class AutoregressiveActionsModel(TFModelV2):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        if action_space != Tuple([Box(low=0.5, high=1.5, shape=(1, ), dtype=np.float32),
                                  Box(low=0.5, high=1.5, shape=(1, ), dtype=np.float32),
                                  Box(low=0.5, high=1.5, shape=(1, ), dtype=np.float32),
                                  Box(low=0.5, high=1.5, shape=(1, ), dtype=np.float32),
                                  Box(low=0.5, high=1.5, shape=(1, ), dtype=np.float32)]):
...
class CorrelatedContinuousActionsEnv(gym.Env):
    ...
    def __init__(self, _):
        ...
        self.action_space = Tuple([Box(low=0.5, high=1.5, shape=(1, ), dtype=np.float32),
                                   Box(low=0.5, high=1.5, shape=(1, ), dtype=np.float32),
                                   Box(low=0.5, high=1.5, shape=(1, ), dtype=np.float32),
                                   Box(low=0.5, high=1.5, shape=(1, ), dtype=np.float32),
                                   Box(low=0.5, high=1.5, shape=(1, ), dtype=np.float32)])
    ...
    def step(self, action):
        ...
        print("{:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f}".format(self.last, a1[0], a2[0], a3[0], a4[0], a5[0], reward))
        self.last = random.choice([0, 1])
