...
class PPOLoss(object):
    def __init__(self,
        ...
        constraint_loss = tf.maximum(0.0,  tf.cast(actions[:, 0]-actions[:, 1], tf.float32)) + \
            tf.maximum(0.0,  tf.cast(actions[:, 1]-actions[:, 2], tf.float32)) + \
            tf.maximum(0.0,  tf.cast(actions[:, 2]-actions[:, 3], tf.float32)) + \
            tf.maximum(0.0,  tf.cast(actions[:, 3]-actions[:, 4], tf.float32))

        self.mean_policy_loss = reduce_mean_valid(-surrogate_loss+constraint_loss)
...
AutoPPOTFPolicy = build_tf_policy(
    name="AutoPPOTFPolicy",
