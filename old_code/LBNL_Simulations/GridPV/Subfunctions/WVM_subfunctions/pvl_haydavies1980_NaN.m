function SkyDiffuse = pvl_haydavies1980_NaN(SurfTilt, SurfAz, DHI, DNI, HExtra, SunZen, SunAz)
...
p.addRequired('DHI');%, @(x) isnumeric(x) && isvector(x) && all(x>=0));
p.addRequired('DNI');%, @(x) isnumeric(x) && isvector(x) && all(x>=0));
p.addRequired('HExtra');%, @(x) isnumeric(x) && isvector(x) && all(x>=0));
p.addRequired('SunZen');%, @(x) isnumeric(x) && all(x<=180) && all(x>=0) && isvector(x));
p.addRequired('SunAz');%, @(x) (isnumeric(x) && all(x<=360) && all(x>=0) && isvector(x)));
...
COSTT2 = cosd(0).*cosd(SunZen) + sind(0).* ...
    sind(SunZen).*cosd(SunAz-0); 
...
AI = DNI./HExtra;
% AI=(1-DHI./(DHI+DNI.*max(COSTT,0))).^2;
